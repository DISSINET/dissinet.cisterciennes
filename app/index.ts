...
var init = () => {
    ...
    const arc = (radius) => d3.arc().outerRadius(radius).innerRadius(0);
        ...
        iconCreateFunction: (cluster) => {
            ...
            markers.forEach((marker) => {
                const gender = genders.find(
                    (g) => g.name === marker.options.gender
                );
                if (gender) {
                    gender.number += 1;
                }
            ...
            return L.divIcon({
                ...
                className:
                    "marker-icon " +
                    (single ? "marker-single" : "marker-cluster"),
    ...
    data.filter((monastery) => monastery.valid).forEach((monastery) => {
...
var prepareData = (): Array<Object> => {
    monasteries
        ...
        .forEach((monastery) => {
            ...
            if (monastery.valid) {
                monastery.ll = [
                    parseFloat(monastery.y),
                    parseFloat(monastery.x),
                ];
            }
        });
